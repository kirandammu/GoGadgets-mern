...
const Navbar = () => {
    ...
    const {user, setUser, logout, setShowUser, navigate, search, setSearch} = useAppContext()
    ...
    return (
        <nav className="flex sticky top-0 items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white z-40">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                <NavLink to={'/'} onClick={()=>setOpen(false)} className={({isActive})=>`${isActive? 'border-b-2' : ''}`} >Home</NavLink>
                <NavLink to={'/products'} onClick={()=>setOpen(false)} className={({isActive})=>`${isActive? 'border-b-2' : ''}`} >All Products</NavLink>
                ...
                {user && (<div onClick={()=>navigate('/cart')} className="relative cursor-pointer">
                    <img src={assets.nav_cart_icon} className='w-6 h-6' alt="" />
