...
const Cart = () => {
    ...
    const getCart = ()=>{
        ...
        for (let i in cartItems){
            const products = product.find((item)=>item._id === i)
            products.quantity = cartItems[i]
        }
    ...
    return product.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
            <div className='flex-1 max-w-4xl'>
                ...
                <button onClick={()=>{navigate('/products'); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-[blue] font-medium">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        <button onClick={() => setShowAddress(!showAddress)} className="text-[blue] hover:underline cursor-pointer">
                            Change
                        </button>
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                ...
                                <p onClick={() => setShowAddress(false)} className="text-[blue] text-center cursor-pointer p-2 hover:bg-blue-500/10">
                                    Add address
                                </p>
                            </div>
                        )}
                ...
                <button className="w-full py-3 mt-6 cursor-pointer rounded-md bg-black text-white font-medium hover:bg-gray-800 transition">
                    Place Order
                </button>
            </div>
        </div>
